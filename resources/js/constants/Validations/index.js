...
const _checkRules = (fieldName, rules1) => {
	...
	for (const key of Object.keys(rules1)) {
		...
		if ((isRuleFn && !rules[key](rules1[key], value)) || (isRegExp && !rules[key].test(value))) {
...
const _addError = (fieldName, rule, value, isFn) => {
	let _nameArray = fieldName.split(/(?=[A-Z])/), _name = '';
	...
	const errMsg = messages[deviceLocale][rule].replace('{0}', name || _name).replace('{1}', value);
...
const ListErrorMessages = (errorList = [], separator = ', ') => {
	...
	for(var i = 0; i < errorList.length; i++) {
		let val = errorList[i];
		_err[val.fieldName] = (val.messages && val.messages[0]) ? val.messages[0] : '';

	...
	useEffect(() => {
		if(currentUser && currentUser.email) {
			...
			if(currentUser.avatar){
				...
				setProfile(createImageFromInitials(200, currentUser.name, getBackgroundColor()))
			}
			...
			{(currentUser && currentUser.email) ? (
				<Fragment>
					...
					<NavDropdown ref={dropper} title={userName} id="basic-nav-dropdown">
						<NavDropdown.Item activeclassname="active" as={NavLink} to='/user/myaccount'>My account</NavDropdown.Item>
						<NavDropdown.Divider />
						{
							showWoopon
							?
							<>
								<NavDropdown.Item activeclassname="active" as={NavLink} to="/business/coupons">My Woopons</NavDropdown.Item>
								<NavDropdown.Divider />
							</>
							: null
						...
						<NavDropdown.Item activeclassname="active" as={NavLink} to="/user/forgotpassword">Change / Forgot password</NavDropdown.Item>
						<NavDropdown.Divider />

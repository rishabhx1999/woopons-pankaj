...
import { Container, Nav, Navbar, NavDropdown, NavItem, Image, Row, Col, Modal, Alert } from 'react-bootstrap';
...
import { TOP_HEADER, RIGHT_HEADER, TOP_HEADER_BUSINESS } from './../../constants/header.js';
...
const mapDispatchToProps = (dispatch) => ({
	onSignOut: () => { dispatch({ type: LOGOUT_USER }) },
	onSubmit: (values) =>
...
const Header = (props) => {
	const { inProgress, appName, currentUser, onSignOut, loginError, loginSuccess, onSubmit, breakpoint = 640  } = props;
  	...
  	useEffect(() => {
  	 ...
  	 useEffect(() => {
	    ...
	    function handleClickOutside(event) {
	    	let toggleButn = document.getElementsByClassName('navbar-toggler')[0]
	    	let _spanTgl = document.getElementsByClassName('navbar-toggler-icon')[0]
	    	// debugger
					...
					<Row className="align-items-center">	
						...
						<Col lg={10} sm={10}>
							<div className="right-navbar">
								<Navbar.Collapse ref={wrapperRef} id="basic-navbar-nav">
									<Nav className="me-auto">
										{
											headerMenus.map((menu, i) => {
												...
												if(hideFrom == 'customer' && currentUser) {
													showMenu = false;
												}
									...
									{currentUser && currentUser.roleId != 1 ?
									 ...
									 :
									 	<>
											...
											<Fragment>
												<Nav.Link activeclassname="active" className="contact-btn" onClick={handleShow} >Sign In
												</Nav.Link>
											</Fragment>
									 	</>
									}
      		...
      		<Modal className="modal-custom" show={show} onHide={handleClose}>
		        ...
		        <Modal.Body>
		        	<div className="popup-content">
		        		...
		        		{loginErr ? <Alert variant="danger">{loginErr}</Alert> : <Fragment /> }
		        		<form onSubmit={submitForm}>
			              	<fieldset>
				                ...
				                <div className="forgot-pass text-right mb-4">
				                	<a onClick={forgotPage} className="red-text">Forgot Password?</a>
				                </div>
